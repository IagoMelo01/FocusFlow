...
export default function MyDayPage() {
  ...
  async function save(values: DailyPlanPayload) {
    ...
    if (!response.ok) {
      ...
      window.alert(payload?.error || t("daily.saveError"));
      return;
    }
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-semibold text-ink">{t("nav.myDay")}</h1>
        <p className="mt-1 text-sm text-muted">{t("daily.subtitle")}</p>
      </div>
          ...
          <Card>
            <div className="mb-4 grid gap-4 md:grid-cols-[220px_1fr]">
              ...
              <div className="rounded-md border border-line bg-brand-50 p-4">
                <p className="text-sm font-semibold text-brand-700">{t("daily.mainPriorities")}</p>
            ...
            {!loading && selectedTasks.length ? (
              ...
              <EmptyState title={t("daily.noSelected")} />
            ) : null}
          </Card>

          <Card>
            <h2 className="mb-3 text-lg font-semibold text-ink">{t("daily.reflection")}</h2>
...
function SelectableTasks({
  ...
  if (!tasks.length) return <p className="text-sm text-muted">{t("daily.nothingHere")}</p>;
